...
impl Inventory {
    pub fn new() -> Self {
        let parrot = Animation {
            metadata: HashMap::from([(
                "Description".to_owned(),
                "The classic Party Parrot.".to_owned(),
            )]),
            frames: PARROT_FRAMES.iter().cloned().map(str::to_owned).collect(),
        };
        Self {
            animations: HashMap::from([("parrot".to_owned(), parrot)]),
        }
    }

    pub fn load_from_path(&mut self, path: impl AsRef<Path>) -> anyhow::Result<()> {
        let files = fs::read_dir(path)?;

        let animations = files
            .filter_map(|file| file.ok())
            .filter(|file| file.file_type().map(|kind| !kind.is_dir()).unwrap_or(false))
            .filter(|file| file.file_name().as_encoded_bytes().ends_with(b".animation"))
            .filter_map(|file| match Animation::load_from_file(file.path()) {
                Ok(animation) => {
                    let filename = file.file_name().into_string();
                    if let Ok(filename) = filename {
                        Some((
                            filename
                                .strip_suffix(".animation")
                                .expect("checked")
                                .to_owned(),
                            animation,
                        ))
                    } else {
                        eprintln!("Failed to get filename of {:?}", file.path());
                        None
                    }
                }
                Err(e) => {
    ...
    pub fn animations(&self) -> impl Iterator<Item = (&String, &Animation)> {
